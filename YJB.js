...
async function runVipCron(arg) {
  ...
  const dayInfo = await fetchAPI(token, ck.ua, '/day_info');
  const isMarketDay = dayInfo && dayInfo.data && dayInfo.data.is_market_day;
  if (!isMarketDay) { $.log('[YJB] 非交易日，跳过行情推送'); return; }
