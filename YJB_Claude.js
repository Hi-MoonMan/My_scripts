...
const STORE_TURNOVER = 'yjb_turnover_baseline'; // 新增: 两市成交额"昨日收盘基准"缓存
...
// ↓↓↓ 新增: 涨跌 emoji 判断（涨=🔴 跌=🟢 平盘=⚪️） ↓↓↓
function trendEmoji(v) {
  const n = Number(v);
  if (isNaN(n) || n === 0) return '⚪️';
  return n > 0 ? '🔴' : '🟢';
}
// ↑↑↑ 新增内容结束 ↑↑↑
...
async function runCron(arg) {
  ...
  const [json, marketLines] = await Promise.all([
    ...
    buildMarketBlock(token, ck.ua), // 改动: 原 fetchVipLines 已重命名/重构为 buildMarketBlock
  ]);
  if (!json) return;
  ...
  if (marketLines.length) {
    $.setdata(JSON.stringify({ lines: marketLines, ts: Date.now() }), STORE_VIP);
  }

  const summary = buildSummary(json);

  // 改动: 标题改为「基金数据·MM-DD HH:mm 『📈X ‖ Y📉』」
  const title = `基金数据·${formatTime(new Date())} 『📈${summary.upCount} ‖ ${summary.downCount}📉』`;

  // 改动: 正文只保留分隔线 + 上证 + 创业板 + 成交额，共4行
  const body = marketLines.join('\n');
...
function buildSummary(json) {
  if (!json || json.code !== 200 || !json.data) {
    return { title: '养基宝', subtitle: '数据为空', body: (json && json.message) || '响应解析失败', upCount: 0, downCount: 0 };
  }
  ...
  // 改动: 场内穿透加涨跌 emoji，且条件反转 —— 净值全部更新完毕才显示 [更新完成]
  const subtitle = `${trendEmoji(todayIncome)}场内穿透 ${signed(todayIncome)} (${signedRate(todayIncomeRate)})${navUpdated ? '  [更新完成]' : ''}`;
...
// ↓↓↓ 新增: 东方财富公共接口，拉取指定 secid 的成交额（f6字段，单位:元） ↓↓↓
function fetchEastmoneyAmount(secid) {
  const url = `https://push2.eastmoney.com/api/qt/stock/get?secid=${secid}&fltt=2&invt=2&fields=f6`;
  const opts = {
    url,
    method: 'GET',
    headers: {
  ...
  return new Promise((resolve) => {
    $task_get(opts, (err, resp, body) => {
      if (err) { $.logErr('[YJB] eastmoney error: ' + err); return resolve(0); }
      try {
        const j = JSON.parse(body);
        resolve((j && j.data && Number(j.data.f6)) || 0);
      } catch (e) { resolve(0); }
    });
  });
}

// 两市成交额(近似) = 上证指数(1.000001,代表沪市) 成交额 + 深证成指(0.399001,代表深市) 成交额
async function fetchTwoMarketTurnover() {
  const [sh, sz] = await Promise.all([
    fetchEastmoneyAmount('1.000001'),
    fetchEastmoneyAmount('0.399001'),
  ]);
  return sh + sz; // 单位: 元
}

// 构建第6行"总成交额"文案，15:00前不带较昨日对比，15:00后带
async function buildTurnoverLine() {
  const total = await fetchTwoMarketTurnover();
  if (!total) return '';

  const now = new Date();
  const todayKey = formatDateKey(now);
  const wanYi = (total / 1e12).toFixed(2) + '万亿';
  let line = `总成交额  ${wanYi}`;

  if (now.getHours() >= 15) {
    let baseline = null;
    try {
      const raw = $.getdata(STORE_TURNOVER);
      if (raw) {
        const obj = JSON.parse(raw);
        if (obj && obj.date && obj.date !== todayKey && obj.total) baseline = obj.total;
      }
    } catch (e) {}

    if (baseline) {
      const diffYi = (total - baseline) / 1e8;
      const sign = diffYi >= 0 ? '+' : '';
      line += `   [较昨日${sign}${diffYi.toFixed(2)}亿]`;
    }

    // 15:00后视为当日收盘最终值，写回缓存供下一交易日比较用
    $.setdata(JSON.stringify({ date: todayKey, total }), STORE_TURNOVER);
  }
...
// 改动: 原 fetchVipLines 重构为 buildMarketBlock，只保留分隔线+上证+创业板+成交额
async function buildMarketBlock(token, ua) {
  ...
  if (idxRes && idxRes.data && Array.isArray(idxRes.data)) {
    const findIdx = (name) => idxRes.data.find(i => i.name === name);
    const sh = findIdx('上证指数');
    const cyb = findIdx('创业板指');

    if (sh) {
      const dir = Number(sh.dir) || 0;
      lines.push(`${trendEmoji(dir)}上证指数 ${Number(sh.v).toFixed(0)}🔛${dir >= 0 ? '+' : ''}${dir.toFixed(2)}%`);
    }
    if (cyb) {
      const dir = Number(cyb.dir) || 0;
      lines.push(`${trendEmoji(dir)}创业板指 ${Number(cyb.v).toFixed(0)}🔛${dir >= 0 ? '+' : ''}${dir.toFixed(2)}%`);
    }
  }

  const turnoverLine = await buildTurnoverLine();
  if (turnoverLine) lines.push(turnoverLine);
...
async function runVipCron(arg) {
  ...
  const lines = await buildMarketBlock(token, ck.ua);
