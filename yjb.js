...
const STORE_TURNOVER = 'yjb_turnover_cache'; // 新增:两市成交额本地缓存(用于计算较昨日变化)
...
async function runCron(arg) {
  ...
  // 基金涨跌徽标拼到标题末尾,例如: 场内穿透 • 09-15 15:05 ‖ 6📈/5📉
  const badge = summary.fundBadge ? ` ‖ ${summary.fundBadge}` : '';
  const title = `场内穿透 • ${formatTime(new Date())}${badge}`;
...
async function runPanel(arg) {
  ...
  // 面板内容里把基金涨跌徽标补回首行,保持信息不丢失
  const fundLine = summary.fundBadge ? `基金涨跌  ${summary.fundBadge}\n` : '';

  panelOutput({
    title: summary.title || '养基宝',
    content: fundLine + summary.body + vipBlock,
...
function buildSummary(json) {
  if (!json || json.code !== 200 || !json.data) {
    return { title: '养基宝', subtitle: '数据为空', body: (json && json.message) || '响应解析失败', fundBadge: '' };
  }
  ...
  // 基金涨跌徽标(挪到标题/面板首行展示,不再放正文)
  const fundBadge = `${upCount}📈/${downCount}📉`;
  // 当前估值前面加涨跌 emoji: 涨🔴 跌🟢 平⚪️
  const subtitle = `${trendEmoji(todayIncome)}当前估值${signed(todayIncome)} (${signedRate(todayIncomeRate)})${navUpdated ? '' : '  [未更新]'}`;
...
// ↓↓↓ 新增内容开始: 两市成交额(公共行情接口,无需 token/签名) ↓↓↓
async function fetchMarketTurnover() {
  // 公共免费接口(东方财富),分别拿上证指数(1.000001)/深证成指(0.399001)的成交额(f6,单位:元)求和
  const url = 'https://push2.eastmoney.com/api/qt/ulist.np/get?fltt=2&secids=1.000001,0.399001&fields=f6';
  const opts = {
    url,
    method: 'GET',
    headers: {
  ...
  const json = await new Promise((resolve) => {
    $task_get(opts, (err, resp, body) => {
      if (err) { $.logErr('[YJB] turnover fetch error: ' + err); return resolve(null); }
      try { resolve(JSON.parse(body)); } catch (e) { resolve(null); }
    });
  });

  if (!json || !json.data || !Array.isArray(json.data.diff)) return null;
  const total = json.data.diff.reduce((s, x) => s + (Number(x.f6) || 0), 0);
  if (!total) return null;

  // 用本地缓存滚动记录"上一交易日"的总成交额,用来算"较昨日变化"
  // 注意:该"昨日基准"取的是"上一交易日脚本最后一次成功运行时"的成交额,
  // 若脚本在收盘后运行(如15:00后),基本等同于昨日全天总成交额;若在盘中运行,则是昨日截至当时的成交额,存在近似误差。
  const todayKey = formatDateKey(new Date());
  let cache = null;
  try { cache = JSON.parse($.getdata(STORE_TURNOVER) || 'null'); } catch (e) {}

  const baseline = (cache && cache.date === todayKey) ? cache.baseline : (cache ? cache.total : null);
  $.setdata(JSON.stringify({ date: todayKey, total, baseline }), STORE_TURNOVER);

  return { total, diff: (baseline != null) ? (total - baseline) : null };
}

function formatTurnoverLine(t) {
  const totalStr = fmtAmount(t.total);
  if (t.diff == null) return `两市成交额${totalStr}`;
  const diffStr = fmtAmount(Math.abs(t.diff));
  return `两市成交额${totalStr}/成交额较昨日${t.diff >= 0 ? '+' : '-'}${diffStr}`;
}
...
async function fetchVipLines(token, ua) {
  const [vipRes, idxRes, turnover] = await Promise.all([
    fetchAPI(token, ua, '/vip_information?page=1'),
    fetchAPI(token, ua, '/market/v1/quote/index-data'),
    fetchMarketTurnover(), // 新增:两市成交额(独立公共接口,失败不影响其他数据)
  ]);

  const lines = [];

  // 上证指数 / 创业板指各占一行,前面加涨跌 emoji;深证成指不再展示,也不再显示"-- 大盘数据 --"副标题
  if (idxRes && idxRes.data && Array.isArray(idxRes.data)) {
    const want = ['上证指数', '创业板指'];
    for (const name of want) {
      const i = idxRes.data.find(x => x.name === name);
      if (!i) continue;
      const dir = Number(i.dir) || 0;
      lines.push(`${trendEmoji(dir)}${name}${Number(i.v).toFixed(0)}${dir >= 0 ? '+' : ''}${dir.toFixed(2)}%`);
    }
  }

  // 两市成交额 / 较昨日变化;若公共接口这次请求失败,turnover 为 null,静默跳过这一行,不影响其他内容
  if (turnover) {
    lines.push(formatTurnoverLine(turnover));
  }

  const rankings = [];
  if (vipRes && vipRes.data && Array.isArray(vipRes.data.list)) {
    for (const item of vipRes.data.list) {
      if (item.type === 'ranking' && Array.isArray(item.buy_data)) {
